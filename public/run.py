...
from public.wechat import sendWechat
...
def interface(task_name,failcount,email_data,env_desc,nosqldb_desc,out_id,test_carryTaskid,wechat):
    startTimer = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    stepcountpass = 0
    stepcountrate = 0
    #获取权重
    case_names_weights={}
    ...
    endTimer = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    ...
    if stepcountall!=0:
        stepcountrate = int(stepcountpass/stepcountall*100)
    #把执行数据全部入库
    #如果正常
    try:
...
接口测试通过数:{}
接口测试总数:{}
接口测试通过率:{}'''.format(task_name,env_desc,startTimer,endTimer,stepcountpass,stepcountall,stepcountrate)
        subject = "接口测试质量"
        url = "http://{}:8000/htmlreport/?task_name={}&find_way=findhtmlreport&carrytaskid={}".format(getLocalIp(),task_name,test_carryTaskid)
        sendWechat(users, content, subject, url)
